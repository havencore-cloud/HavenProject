...
def get_token_profile(symbol: str, mint: str, include_logs: bool = False) -> dict:
    ...
    # --- Step 1: Try getting full token metrics (price, volume) ---
    ...
    # --- Step 2: Get liquidity from Jupiter indexed pools ---
    token_info["liquidity_usd"] = estimate_token_liquidity(mint)

    # --- Step 3: Try to get name and decimals from Jupiter token list ---
    metadata = get_jupiter_token_metadata(mint)
    if metadata:
        token_info["name"] = metadata.get("name", symbol)

    # --- Step 4: Routability check (even if we fallback on price) ---
    token_info["routable"] = is_token_routable_to_sol(mint)

    # --- Step 5: Risk score (simple version for now) ---
    if token_info["volume_24h"] < 5000 or token_info["liquidity_usd"] < 1000:
